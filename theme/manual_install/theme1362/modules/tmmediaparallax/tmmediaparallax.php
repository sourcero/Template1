...
class Tmmediaparallax extends Module
{
    ...
    protected function checkVideoWebm()
    {
        if (Tools::isEmpty(Tools::getValue('video_webm'))) {
            ...
            $info = new SplFileInfo(Tools::getValue('video_webm'));
            if ($info->getExtension() != 'webm') {
    ...
    protected function checkVideoMp()
    {
        if (Tools::isEmpty(Tools::getValue('video_mp4'))) {
            ...
            $info = new SplFileInfo(Tools::getValue('video_mp4'));
    ...
    protected function saveLayout()
    {
        ...
        switch ($type) {
            case 1:
                $image_url = explode(__PS_BASE_URI__, Tools::getValue('image'));
                $layout->image = $image_url[1];
                break;
            case 2:
                $image_url = explode(__PS_BASE_URI__, Tools::getValue('image'));
                $video_mp4_url = explode(__PS_BASE_URI__, Tools::getValue('video_mp4'));
                $video_webm_url = explode(__PS_BASE_URI__, Tools::getValue('video_webm'));
                $layout->image = $image_url[1];
    ...
    {
        $info = new SplFileInfo($file_link);
            ...
            if ($mobile) {
                ...
                foreach ($layouts as $layout_key => $layout) {
                    if ((int)$layout['type'] == 2) {
                        $layout['image_type'] = $this->getFileType(__PS_BASE_URI__ . $layout['image']);
